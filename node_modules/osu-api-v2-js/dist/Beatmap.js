...
(function (Beatmap) {
    ...
    async function getUserScore(beatmap, user, config) {
        ...
        return await this.request("get", `beatmaps/${getId(beatmap)}/scores/users/${getId(user)}`, { legacy_only: config?.legacy_only, mode, mods: config?.mods, type: config?.type });
    }
    ...
    async function getUserScores(beatmap, user, config) {
        const mode = config?.ruleset !== undefined ? Ruleset[config.ruleset] : undefined;
        const response = await this.request("get", `beatmaps/${getId(beatmap)}/scores/users/${getId(user)}/all`, { legacy_only: config?.legacy_only, mode, mods: config?.mods, type: config?.type });
    ...
    */
    ...
    async function getScores(beatmap, config) {
        const mode = config?.ruleset !== undefined ? Ruleset[config.ruleset] : undefined;
        const response = await this.request("get", `beatmaps/${getId(beatmap)}/scores`, { legacy_only: config?.legacy_only, mode, mods: config?.mods, type: config?.type });
        return response.scores; // It's the only property
    }
     ...
     */
    async function getSoloScores(beatmap, config) {
        const mode = config?.ruleset !== undefined ? Ruleset[config.ruleset] : undefined;
        const response = await this.request("get", `beatmaps/${getId(beatmap)}/solo-scores`, { legacy_only: config?.legacy_only, mode, mods: config?.mods, type: config?.type });
        return response.scores; // It's the only property
    }
